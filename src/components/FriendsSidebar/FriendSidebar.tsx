...
const FriendSidebar = () => {
  const {
    ...
    getMyFriends,
    myFriends,
    loading: { unfollowUser: loadingUnfollowUser }
  } = useUserStore()
      ...
      {myFriends.map((friend, index) => {
        const popoverContent = (
          <div className='w-85 flex flex-col'>
            <div
              onClick={() => message.info('Đang phát triên tính năng...')}
              className='flex items-center gap-x-3 p-2 hover:bg-gray-100 rounded-md cursor-pointer transition-all'
            >
            ...
            {loadingUnfollowUser ? (
              ...
              <div
                onClick={() => handleUnFollowUser(friend._id as string)}
                className='flex items-start gap-x-3 p-2 hover:bg-gray-100 rounded-md cursor-pointer transition-all mt-1'
              >
        ...
        return (
          ...
          >
            ...
            <Popover
              ...
              arrow={false}
              overlayInnerStyle={{ padding: '8px', borderRadius: '8px', boxShadow: '0 2px 12px rgba(0,0,0,0.2)' }}
            >
              <div
                className='w-8 h-8 rounded-full flex items-center justify-center hover:bg-gray-200 transition-all cursor-pointer'
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                }}
