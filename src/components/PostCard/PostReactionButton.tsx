...
const PostReactionButton = () => {
  ...
  const activeReaction = REACTIONS.find((r) => r.id === active);
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex cursor-pointer items-center gap-3 rounded-full border border-[#E4E6EB] bg-white px-3 py-2 shadow-[0_4px_16px_rgba(0,0,0,0.15)]">
          {REACTIONS.map((r) => (
            ...
            >
              <ReactionPickerIcon id={r.id} emoji={r.emoji} />
            </button>
          ))}
        </div>
      </div>
      ...
      <button
        ...
        className={`flex cursor-pointer items-center gap-1.5 rounded-md px-1.5 py-1 transition-colors ${active ? activeReaction?.activeText : "text-[#65676B] hover:bg-[#F2F2F2]"
          }`}
      >
        {active ? (
          <ActiveReactionIcon id={active} />
        ) : (
