        ...
        <div className="flex flex-col gap-y-1">
          {listFriends && listFriends.map((item, index) => (
            <div key={index} className="flex items-start gap-x-3 p-2 hover:bg-gray-100 rounded-lg cursor-pointer transition-all">
              <img className="w-[60px] h-[60px] rounded-full object-cover bg-gray-300 flex-shrink-0" src="" alt="" />
              <div className="flex flex-col flex-1">
                <span className="font-semibold text-[15px] text-black">{item.first_name + " " + item.last_name}</span>
                <div className="flex items-center gap-x-1 mt-0.5">
                  <div className="flex items-center -space-x-1">
                    <img className="w-[18px] h-[18px] rounded-full border-2 border-white bg-gray-300 relative z-10" src="" alt="" />
                    <img className="w-[18px] h-[18px] rounded-full border-2 border-white bg-gray-400 relative z-0" src="" alt="" />
                  </div>
                  <span className="text-[13px] text-gray-500">4 bạn chung</span>
                </div>
                <div className="flex items-center gap-x-2 mt-3 w-full">
                  <button className="flex-1 bg-[#1877F2] text-white font-semibold py-[6px] rounded-md hover:bg-[#166FE5] transition-all text-[15px] cursor-pointer">Thêm bạn bè</button>
                  <button className="flex-1 bg-[#E4E6E9] text-black font-semibold py-[6px] rounded-md hover:bg-[#D8DADF] transition-all text-[15px] cursor-pointer">Gỡ</button>
                </div>
              </div>
            </div>
          ))}
