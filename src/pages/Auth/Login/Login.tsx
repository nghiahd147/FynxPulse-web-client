  ...
  return (
    <div className='w-full h-screen overflow-x-hidden bg-gray-100 flex items-center justify-center'>
      ...
      <div className='sm:ml-10 w-100 bg-white shadow-md border-gray-200 border rounded-md p-4 flex flex-col gap-y-3'>
        <Form onFinish={onFinish} onFinishFailed={onFinishFailed} autoComplete='on' className='w-full'>
          <Form.Item name='email' rules={[{ required: true, message: 'Vui lòng nhập email!' }]}>
            <Input style={{ padding: '12px' }} placeholder='Email hoặc số điện thoại' type='email' />
          </Form.Item>
          <Form.Item
            name='password'
            rules={[
          ...
          >
            <Input.Password style={{ padding: '12px' }} placeholder='Mật khẩu' type='password' />
          </Form.Item>
          <Button loading={isLoading} message='Đăng nhập' />
        </Form>
        <Link className='text-sm text-red-600 text-center hover:underline' to={'/login'}>
          Quên mật khẩu?
        </Link>
        <Divider className='bg-gray-200' style={{ marginTop: '2px', marginBottom: '3px' }} />
        <Link
          to={'/register'}
        ...
        >
          <img src='/google.png' alt='icon-google' className='w-5 h-5 ml-2 mt-0.5' />
