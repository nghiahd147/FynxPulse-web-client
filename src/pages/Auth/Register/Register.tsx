...
const Register = () => {
  ...
  const onFinish = async (values: Users) => {
    const payload = {
      ...values,
      date_of_birth: convertYearMonthDay(values.date_of_birth || new Date().toISOString())
    }
      ...
      <div className='bg-white shadow-md border-gray-200 border rounded-md p-4 flex flex-col gap-y-5'>
        ...
        >
          <div className='flex items-center gap-x-2'>
            <Form.Item name='first_name' rules={[{ required: true, message: 'Vui lòng họ' }]}>
              <Input style={{ padding: '12px' }} placeholder='Họ' type='text' />
            </Form.Item>
            <Form.Item name='last_name' rules={[{ required: true, message: 'Vui lòng tên' }]}>
          ...
          <Form.Item
            ...
            className='w-full'
          >
            <DatePicker style={{ padding: '12px' }} className='py-3 px-3 w-full' placeholder='Ngày sinh' />
          </Form.Item>
          <Button loading={isLoading} message='Đăng ký' />
        </Form>
        <Link className='text-sm text-black text-center hover:underline' to={'/login'}>
