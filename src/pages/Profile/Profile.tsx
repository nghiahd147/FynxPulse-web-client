...
import { ChevronDown, ChevronUp, Ellipsis, Pencil, UserCheck, UserRoundPlus } from 'lucide-react'
  ...
  return (
    <div className='w-full bg-bgPrimary'>
      ...
      <div className='w-full flex flex-col items-center bg-white border-b border-bgPrimary z-10 relative'>
        ...
        <div className={`relative z-10 ${location.pathname.startsWith('/profile') ? 'w-300' : 'w-[90%]'}`}>
          {/* Background */}
          <div className='w-full h-116.25 flex'>
            <div className='flex-1 mx-auto relative overflow-hidden rounded-b-2xl'>
              <img
                src={profileUser.profile_picture_url || '/anh_nen_mac_dinh_2.jpg'}
                alt='bg-user'
              ...
              <div className='flex items-center justify-between'>
                <div className='flex flex-col my-2'>
                  ...
                  <div className='flex items-center gap-x-2 font-bold'>
                    <span
                      onClick={redirectToPageFollowing}
                      className='cursor-pointer hover:text-blue-400 hover:underline transition-all ease-in'
                    >
                      {profileUser.following_count} đang theo dõi
                    </span>
                    <span>•</span>
                    <span
                      onClick={redirectToPageFollowers}
                      className='cursor-pointer hover:text-blue-400 hover:underline transition-all ease-in'
                    >
                      {profileUser.followers_count} người theo dõi
                    </span>
                  </div>
                </div>
          ...
          <div className='h-15 flex items-center justify-between border-t border-[#ced0d4]'>
            <div className='h-full flex items-center gap-x-1'>
              <Link
                ...
                className={`relative flex items-center justify-center h-12 px-4 font-semibold rounded-md cursor-pointer transition-colors ${
                  location.pathname === `${basePath}/${profileUser?.user_name}`
                    ? 'text-[#1877F2]'
                    : 'text-[#65676B] hover:bg-[#F2F2F2]'
                } after:absolute after:-bottom-1.5 after:left-0 after:w-full after:h-0.75 after:bg-[#1877F2] after:transition-transform after:duration-300 after:ease-out ${
                  location.pathname === `${basePath}/${profileUser?.user_name}`
                    ? 'after:scale-x-100'
                    : 'after:scale-x-0'
                }`}
              ...
              <Link
                ...
                className={`relative flex items-center justify-center h-12 px-4 font-semibold rounded-md cursor-pointer transition-colors ${
                  location.pathname === `${basePath}/${profileUser?.user_name}/images`
                    ? 'text-[#1877F2]'
                    : 'text-[#65676B] hover:bg-[#F2F2F2]'
                } after:absolute after:-bottom-1.5 after:left-0 after:w-full after:h-0.75 after:bg-[#1877F2] after:transition-transform after:duration-300 after:ease-out ${
                  location.pathname === `${basePath}/${profileUser?.user_name}/images`
              ...
              <Link
                ...
                className={`relative flex items-center justify-center h-12 px-4 font-semibold rounded-md cursor-pointer transition-colors ${
                  location.pathname === `${basePath}/${profileUser?.user_name}/friends`
                    ? 'text-[#1877F2]'
                    : 'text-[#65676B] hover:bg-[#F2F2F2]'
                } after:absolute after:-bottom-1.5 after:left-0 after:w-full after:h-0.75 after:bg-[#1877F2] after:transition-transform after:duration-300 after:ease-out ${
                  location.pathname === `${basePath}/${profileUser?.user_name}/friends`
              ...
              <Link
                ...
                className={`relative flex items-center justify-center h-12 px-4 font-semibold rounded-md cursor-pointer transition-colors ${
                  location.pathname === `${basePath}/${profileUser?.user_name}/posts`
                    ? 'text-[#1877F2]'
                    : 'text-[#65676B] hover:bg-[#F2F2F2]'
                } after:absolute after:-bottom-1.5 after:left-0 after:w-full after:h-0.75 after:bg-[#1877F2] after:transition-transform after:duration-300 after:ease-out ${
                  location.pathname === `${basePath}/${profileUser?.user_name}/posts`
      ...
      {/* Body Profile */}
      <div
        className={`${location.pathname.startsWith('/profile') ? 'w-300' : 'w-[90%]'} flex justify-between gap-x-5 mx-auto py-4`}
      >
        <Outlet />
      </div>
    </div>
