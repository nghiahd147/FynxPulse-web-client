    ...
    <div className='w-full h-full flex flex-col shadow-md bg-white'>
      ...
      <div className='flex border-b border-gray-200 px-3'>
        <button
          ...
          className={`relative px-4 py-3 cursor-pointer transition-colors duration-300 ease-out ${
            activeTab === 'following' ? 'text-blue-500 font-semibold' : 'text-gray-500 font-medium hover:text-gray-700'
          }`}
        >
          Người theo dõi
          <span
            className={`absolute bottom-0 inset-x-4 h-0.5 rounded-full bg-blue-500 origin-center transition-transform duration-300 ease-in-out ${
              activeTab === 'following' ? 'scale-x-100' : 'scale-x-0'
            }`}
          />
        </button>
        <button
      ...
      <div ref={friendRef} className='w-full my-4 grid grid-cols-2 gap-4 px-3 py-1'>
        {yourFriends.length !== 0 && activeTab == 'following' ? (
          ...
          <i className='w-full text-center text-gray-500 col-span-2'>Chưa có người theo dõi...</i>
        )}
      </div>
    </div>
  )
